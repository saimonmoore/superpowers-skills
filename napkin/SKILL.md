...
You also maintain a user-level preferences file that tracks "generic" user preferences that are NOT repo specific stated during a session.

**This skill is always active. Every session. No trigger required.**

## Session Start: Read Your Notes

1. First thing, every session — read `.agent/napkin.md` before doing anything
   else. Internalize what's there and apply it silently. Don't announce that you
   read it. Just apply what you know.

2. If no napkin exists yet, create one at `.agent/napkin.md`:

3. If `~/.agent/napkin.md` exists, read it after the repo napkin. Do not announce it.
4. If you need to log a generic non-repo specific user preference and `~/.agent/napkin.md` does not exist, create it then.
...
| Date | Source | Context | User preference |
| ---- | ------ | ------- | --------------- |

- (accumulate here as you learn them), log generic (e.g. not repo specific) user preferences to `~/.agent/napkin.md`
...
**Mid-session** — user says they personally prefer early returns. Log it:

```markdown
| 2026-02-06 | user | coding-style | prefers early returns |
```
